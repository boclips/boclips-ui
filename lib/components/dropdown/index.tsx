...
import ArrowDownIcon from './resources/down-icon.svg?react';
...
export const Dropdown = ({
  ...
  }, [filterBySearch, inputTextValue, options, showSearch]);

  const handleMultipleValueUpdate = useCallback(() => {
  ...
  }, [mode, onUpdate, selectedOptions, values]);

  // 1. Update dropdown options:
  useEffect(() => {
    setDropdownOptions(updateDropdownOptions());
  }, [inputTextValue, options, updateDropdownOptions]);
  ...
  useEffect(() => {
    if (defaultValue) {
      if (mode === 'single') {
        setSingleValue(dropdownOptions?.find(it => it.value === defaultValue));
      }
      if (mode === 'multiple') {
  ...
  }, [handleMultipleValueUpdate, selectedOptions, values]);
